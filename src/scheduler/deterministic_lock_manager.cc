...
int DeterministicLockManager::Lock(TxnProto* txn, set<Key> skip_keys) {
  int not_acquired = 0;
  uint32 origin = txn->origin_replica();
...
int DeterministicLockManager::Lock(TxnProto* txn, Key only_lock_key) {
	...
	deque<KeysList>::iterator it;
	for(it = key_requests->begin(); it != key_requests->end() && it->key != only_lock_key; ++it) {
	}

	deque<LockRequest>* requests;
	if (it == key_requests->end()) {
	    requests = new deque<LockRequest>();
	    key_requests->push_back(KeysList(only_lock_key, requests));
	} else {
	    requests = it->locksrequest;
	}
	...
	if (requests->empty() || txn != requests->back().txn) {
	  ...
	  if (txn_waits_.find(txn) == txn_waits_.end()) {
		  ...
		  txn_waits_[txn] = txn_waits_[txn] + not_acquired;
	  }
	  return not_acquired;
}
